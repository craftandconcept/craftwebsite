    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-9">
                ...
                <form method="POST" enctype="multipart/form-data" class="project-form" action="{{route('teams.update',$team->id)}}">
                    @csrf
                    {{method_field('PUT')}}
                    <div class="form-group">
                        <label for="name">First Name</label>
                        <input required name="name"  value="{{$team->name}}" id="name" class="form-control" type="text">
                    </div>
                    <div class="form-group">
                        <label for="last_name">Last Name</label>
                        <input name="last_name"  value="{{$team->last_name}}" id="last_name" class="form-control" type="text">
                    </div>

                    <div class="form-group">
                        <label for="title">Title</label>
                        <input name="title" value="{{$team->title}}" id="title" class="form-control" type="text">
                    </div>

                    <div class="form-group">
                        <label for="description">Description</label>
                        <input name="description" value="{{$team->description}}" class="form-control" type="text">
                    </div>


                    <p class="h5">Choose Main image</p>
                    <div class="input-group mb-3 increment">
                        <img src="{{ URL::to('/') }}/{{$team->main_image}}" alt="" style="height: 37px;">
                        <div class="input-group-prepend">
                            <span class="input-group-text">File</span>
                        </div>
                        <div class="custom-file">
                            <input type="file" name="main_image" class="custom-file-input">
                            <input type="hidden" name="isset_main_image" value="{{$team->main_image}}" class="form-control">
                            <label class="custom-file-label">{{$team->main_image}}</label>
                        </div>
                    </div>
                    ...
                    @foreach ($team->images as $i_key => $photo)
